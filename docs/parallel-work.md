...
3. Spin up a worktree for isolation (recommended, not required):
   ...
   cd E:\Dev\worktrees\3dprinter\chunk-<n>-<short-name>
   ```
...
**Acceptance criteria:**
- Double-clicking `start.ps1` boots the app end-to-end on the kantoor workstation.
...
| **Firecrawl fallback** | `$FIRECRAWL_API_KEY` env var (lives on the kantoor workstation). POST to `https://api.firecrawl.dev/v1/scrape` with `{url, formats: ['extract'], extract: {schema: {hex: 'string', effects: 'string[]'}}}`. Confidence: low. Use only when no brand resolver matched. |
